...
import {
  ...
  openProductWithDeliveryOptions,
  addToCartAndWaitForUpdate,
  navigateToFirstProductWithDelivery,
} from './utils/waits'
...
test.describe('Delivery Options', () => {
  ...
  test('should display delivery summary on product cards', async ({ page }) => {
    ...
    const deliverySection = page.locator(
      '[data-testid="delivery-section"], [data-testid="delivery-options"], [data-testid="shipping-section"], [data-testid="shipping-options"]'
    )
  ...
  test('should allow selecting different delivery options', async ({ page }) => {
    ...
    for (let i = 0; i < radioCount; i++) {
      ...
      if (value !== initialValue && !isDisabled) {
        ...
        if (initialValue) {
          const previousRadio = deliverySection.locator(
