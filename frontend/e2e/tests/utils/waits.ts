...
export async function waitForProductsLoaded(page: Page) {
  ...
  await Promise.all([
    page.waitForResponse((resp) => resp.url().includes('/products') && resp.ok(), {
      timeout: 10000,
    }),
...
export async function waitForDeliveryLoaded(page: Page, opts: { productId?: string } = {}) {
  ...
  await Promise.all([waitForDeliveryNetwork(page, opts), waitForDeliveryUI(page)])
}
...
export async function openFirstProductWithDeliveryByClicking(page: Page, maxCardsToTry = 5) {
  ...
  for (let i = 0; i < total; i++) {
    await Promise.all([page.waitForURL('**/products/**'), cards.nth(i).click()])
...
export async function navigateToKnownProductWithDelivery(page: Page) {
  ...
  await Promise.all([page.waitForURL('**/products/**'), firstProductCard.click()])
