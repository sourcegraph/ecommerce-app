...
def seed_products(session: Session, products: list, category_map: dict):
    ...
    for product_data in products:
        ...
        if existing_product:
            print(
                f"Product '{product_data['title']}' already exists (ID: {product_data['id']})"
            )
            continue
...
def seed_delivery_options(session: Session) -> list[DeliveryOption]:
    ...
    for option in delivery_options:
        ...
        existing = session.exec(
            select(DeliveryOption).where(DeliveryOption.name == option.name)
        ).first()
        if existing:
...
def assign_delivery_options_to_products(
    session: Session, delivery_options: list[DeliveryOption]
):
    ...
    for i, product in enumerate(products):
        ...
        print(
            f"Assigned delivery options to '{product.title}': {', '.join(option_names)}"
        )
...
def seed_database(custom_engine=None):
    ...
    print(
        f"   - Delivery options: {len(delivery_options) if 'delivery_options' in locals() else 0}"
    )
    print("   - All product images downloaded and stored as BLOBs")
    print(
        "   - Delivery options deterministically assigned to products (by product ID)"
    )
